...
class UserNewsList extends Component {
    ...
    handleChange = (e) => {
        if(e.target.name == 'date_filter'){
            ...
            this.state.date_filter = e.target.value;
            this.getList();
        }
    }
    ...
    goNewsPage = (go_id) => {
        if(go_id != 0){
            jsCookie.set('view_news_id', go_id);
            jsCookie.set('user_action', 'news_details');
                ...
                <div className="content_page">
                    ...
                    <div className="item_group">
                        {this.state.items && this.state.items.length > 0 && this.state.items.map((k, v) => {
                            return <div key={k.id} className="item_box" onClick={e => this.goNewsPage(k.id)}><div className="item_image" style={{backgroundImage:`url(${k.cover_image})`}}></div><div className="item_content"><div className="item_title">{k.title}</div><div className="item_desc">{k.description}</div><div className="item_date">{k.publish_date.replace(/T/, ' ').replace(/\..+/, '')}</div></div><div className="clear"></div></div>
                        })}
                    </div>
                </div>
                <style jsx>{`
